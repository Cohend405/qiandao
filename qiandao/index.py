...
def getCpdailyApis(user):
    apis = {}
    user = user['user']
    idsUrl = "http://authserver.hnuahe.edu.cn/authserver"
    ampUrl = "https://hnuahe.campusphere.net/wec-portal-mobile/client"
    ampUrl2 = ""
...
#         if send != '':
#             log('正在发送邮件通知。。。')
#             res = requests.post(url='https://api.ruoli.cc/sendMail',
#                                 data={'title': '若离今日校园通知', 'content': msg, 'to': send}, verify=not debug)
#             code = res
#             if code == 'ok':
#                 log('发送邮件通知成功。。。')
#             else:
